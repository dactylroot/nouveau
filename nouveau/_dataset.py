...
class _ArtDataset():
    ...
    def to_torch(self):
        ...
        class _TorchDataset(torch.utils.data.Dataset, ArtClass):
            ...
            def __getitem__(self, idx):
                ...
                filename = self.index.iloc[idx].filename
                self._ensure_downloaded(filename)
                image = _io.imread(self.storage_path / self._storage_jpg / filename)
                if image.ndim == 2:
    ...
    def sample(self):
        ...
        filename = _random.choice(self.index['filename'].tolist())
        self._ensure_downloaded(filename)
    ...
    def _ensure_downloaded(self, filename):
        _file = self.storage_path / self._storage_jpg / filename
        if _file.is_file():
            return
        ...
        try:
            _request.urlretrieve(f"{self._download_base_url}/{filename}", _file)
        except Exception as e:
            _file.unlink(missing_ok=True)
            raise RuntimeError(f"couldn't download {filename}: {e}") from e

    def _self_validate(self):
        print(f"{type(self).__name__}: {len(self.index)} images in index.")
